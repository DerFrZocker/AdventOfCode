...
public class Part2 {
    ...
    private record CpyInstruction(boolean isRegistry, char registry, int value, char target) implements Instruction {
        ...
        public int execute() {
            if (isRegistry) {
                REGISTRY.put(target, REGISTRY.computeIfAbsent(registry, a -> 0L));
            } else {
                REGISTRY.put(target, (long) value);
            }
    ...
    private record IncInstruction(char registry) implements Instruction {
        ...
        @Override
        public int execute() {
            REGISTRY.compute(registry, (a, b) -> b == null ? 1 : b + 1L);
    ...
    private record JnzInstruction(boolean isRegistry, char registry, int value, int line) implements Instruction {
        ...
        public int execute() {
            if (isRegistry) {
                if (REGISTRY.computeIfAbsent(registry, a -> 0L) != 0) {
                    return line;
                }
