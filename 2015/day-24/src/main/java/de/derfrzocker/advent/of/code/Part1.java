...
public class Part1 {
    ...
    private static List<Integer> solve(int target, List<Integer> current, int currentSum, List<Integer> weights, int index) {
        ...
        for (int i = index; i < weights.size(); i++) {
            ...
            if (currentSum + weight == target) {
                ...
                boolean canSolve = canSolveRest(target, 0, new ArrayList<>(), newWeights, 0);
                if (canSolve) {
                    if (currentBest == null) {
                        currentBest = result;
    ...
    private static boolean canSolveRest(int target, int currentSum, List<Integer> current, List<Integer> weights, int index) {
        for (int i = index; i < weights.size(); i++) {
            int weight = weights.get(i);
            if (currentSum + weight > target) {
                continue;
            ...
            boolean canSolve = canSolveRest(target, currentSum + weight, result, weights, i + 1);
