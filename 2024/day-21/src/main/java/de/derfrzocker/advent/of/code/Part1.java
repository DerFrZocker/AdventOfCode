...
public class Part1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (String line : lines) {
            ...
            Position firstPostion = directionalKeypad.get('A');
            List<Character> firstRoboter = new ArrayList<>();
            for (char c : numericMoves) {
                Result result = pressDirectional(c, firstPostion);
                firstPostion = result.newPos();
                firstRoboter.addAll(result.characters());
            }

            Position secondPostion = directionalKeypad.get('A');
            List<Character> secondRoboter = new ArrayList<>();
            for (char c : firstRoboter) {
                Result result = pressDirectional(c, secondPostion);
                secondPostion = result.newPos();
                secondRoboter.addAll(result.characters());
            }

            System.out.println("Size: " + numericMoves.size() + " " + toString(numericMoves));
            System.out.println("Size: " + firstRoboter.size() + " " + toString(firstRoboter));
            System.out.println("Size: " + secondRoboter.size() + " " + toString(secondRoboter));
            long tmp = Math.multiplyExact(secondRoboter.size(), Integer.parseInt(line.substring(0, line.length() - 1)));
    ...
    public static List<Character> moveNumeric(char c) {
        ...
        if (posKeypad.x() == 0 && position.y() == 3) {
            ...
            if (xDif == 0) {
                ...
                if (yDif < 0) {
                    directions = new Direction[]{Direction.UP, Direction.RIGHT};
                } else {
                    directions = new Direction[]{Direction.DOWN, Direction.RIGHT}; // Better
                }
            }
